    ...
    async ({ event, step }) => {
        try {
            ...
            const analysis = await step.run("analyze-message", async () => {
                try {
                    // const model = genAI.models.generateContent({ model: "gemini-2.0-flash", contents: "Explain how AI works in a few" });
          ...
          Required JSON structure:
          {
            ...
            5. Considers safety and well-being`;
                    ...
                    const responseText = genAI.models.generateContent({ model: "gemini-2.0-flash", contents: prompt });
