...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [testRes, moodRes] = await Promise.all([
        ...
        const [testDataJson, moodDataJson] = await Promise.all([
          testRes.json(),
          moodRes.json(),
        ]);

        if (!testRes.ok) {
          setError(testDataJson.error || "Failed to fetch test data");
          return;
        }
        if (!moodRes.ok) {
          setError(moodDataJson.error || "Failed to fetch mood data");
          return;
        }

        console.log("test data", testDataJson);
        console.log("mood data", moodDataJson);

        setTestData(testDataJson.data);
        setMoodData(moodDataJson.data);
      } catch (err) {
    ...
    // Run on first mount OR when modal just closed
  ...
  const wellnessStats = [
    {
      title: "Mood Score",
      value: moodData?.score != null ? `${moodData.score}` : "No data",
      icon: Brain,
...

    {
      ...
      value: testData?.score != null ? `${testData.score} / 30` : "No data",
      icon: Brain,
      color: "text-rose-500",
      bgColor: "bg-rose-500/10",
      description: testData?.level || "No data",
    },
  ];
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid gap-3">
                    ...
                    <Button
                      ...
                      className={cn(
                        "w-full justify-between items-center p-6 h-auto group/button cursor-pointer",
                        "bg-gradient-to-r from-[#412c40] to-[#3c0f3a] hover:from-[#958495] hover:to-[#3c0f3a]",
                        "transition-all duration-200 group-hover:translate-y-[-2px]"
                      )}
                    ...
                    <Button
                      ...
                      className={cn(
                        "w-full justify-between items-center p-6 h-auto group/button cursor-pointer",
                        "bg-gradient-to-r from-[#0f4c81] to-[#1c6aa4] hover:from-[#3d7ea6] hover:to-[#1c6aa4]",
                        "transition-all duration-200 group-hover:translate-y-[-2px] rounded-xl shadow-md hover:shadow-lg"
                      )}
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div className="text-left">
                          <div className="font-semibold text-white text-lg">Take a Test</div>
                          <div className="text-xs text-white/80">Begin your Postpartum Well-Being Check</div>
                        </div>
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <Button
                        variant="outline"
                        className={cn(
                          "flex flex-col h-[120px] px-4 py-3 group/mood hover:border-primary/50 cursor-pointer",
                      ...
                      <Button
                        variant="outline"
                        className={cn(
                          "flex flex-col h-[120px] px-4 py-3 group/ai hover:border-primary/50 cursor-pointer",
                          "justify-center items-center text-center",
                          "transition-all duration-200 group-hover:translate-y-[-2px]"
                        )}
            ...
            <Card className="border-primary/10">
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div>
                    <CardTitle>Today&lsquo;s Overview</CardTitle>
                    <CardDescription>
